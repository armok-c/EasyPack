...
export function EnvironmentWorkspace({ projectPath, state, busy, error, recoveryBlocked, recoveryError, migrationRequired, migrationDraft, onRefresh, onCreate, onCapture, onCaptureMany, onDetail, onCopy, onDelete, onDeleteMany, onMigrate, onPlan, onApply, onPlanUndo, onUndo, progress = {} }: EnvironmentWorkspaceProps) {
  ...
  const deleteSelected = async () => {
    ...
    await run(async () => {
      ...
      const failed = batch.results.filter((result) => !result.success).length;
      const succeeded = batch.results.length - failed;
      const deleted = new Set(batch.results.filter((result) => result.success).map((result) => result.environmentId));
     ...
     <div className="space-y-1.5">{environments.map((environment) => {
       ...
       const statusText = environmentProgress
         ? environmentProgress.status === "running" ? `${environmentProgress.kind === "capture" ? "更新" : "应用"}处理中 ${environmentProgress.percent}%`
           : `${environmentProgress.kind === "capture" ? "更新" : "应用"}${environmentProgress.status === "success" ? "成功" : "失败"}${environmentProgress.status === "success" ? " 100%" : ""}`
         : waiting ? "等待中" : "就绪";
       return <div key={environment.id} data-environment-row={environment.id} className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 rounded-md border border-border px-3 py-2 sm:flex-nowrap">
         <Checkbox aria-label={`选择环境 ${environment.name}`} checked={selectedIds.includes(environment.id)} onCheckedChange={(checked) => setSelectedIds((current) => checked ? [...current, environment.id] : current.filter((id) => id !== environment.id))} disabled={!canEdit} />
         <div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{environment.name}</p><span className="whitespace-nowrap text-xs text-muted-foreground">{environment.fileCount} 个文件</span></div>
         <div className="order-last flex min-w-0 basis-full flex-col gap-1 sm:order-none sm:basis-auto sm:flex-1" data-progress-status={environmentProgress?.status ?? (waiting ? "waiting" : "idle")}><span className="text-xs text-muted-foreground">{statusText}</span>{environmentProgress && <div role="progressbar" aria-label={`${environment.name} ${environmentProgress.kind === "capture" ? "更新" : "应用"}进度`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={environmentProgress.percent} className="h-1.5 overflow-hidden rounded-full bg-muted"><div className={cn("h-full transition-[width]", environmentProgress.status === "failed" ? "bg-red-400" : environmentProgress.status === "success" ? "bg-emerald-400" : "bg-primary")} style={{ width: `${environmentProgress.percent}%` }} /></div>}</div>
          <Button variant="ghost" size="sm" className="shrink-0" onClick={() => setDetailTarget({ id: environment.id, name: environment.name })} disabled={!canEdit}><Eye className="size-4" />查看</Button>
       </div>;
     })}</div>
