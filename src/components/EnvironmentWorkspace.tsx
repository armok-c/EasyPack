...
export function EnvironmentWorkspace({ projectPath, state, busy, error, recoveryBlocked, recoveryError, migrationRequired, migrationDraft, onRefresh, onCreate, onCapture, onCopy, onMigrate, onPlan, onApply, onPlanUndo, onUndo }: EnvironmentWorkspaceProps) {
  ...
  return <div className="space-y-5">
     <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-base font-medium">项目环境</h3><p className="mt-1 text-xs text-muted-foreground">按项目保存配置文件快照，应用前会先列出文件变更。</p></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => setPathDialogOpen(true)} disabled={!canEdit || !state}><FolderSync className="size-4" />受管文件清单</Button><Button size="sm" onClick={() => setNewDialogOpen(true)} disabled={!canEdit}><Plus className="size-4" />新建环境</Button>{state?.undoAvailable && <Button variant="outline" size="sm" onClick={() => void prepareUndoPlan()} disabled={!canEdit}><RotateCcw className="size-4" />撤销上次应用</Button>}</div></div>
    {state && <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground"><span>受管文件 {state.managedPaths.length} 个</span><span>环境 {environments.length} 个</span></div>}
    ...
    <div className="space-y-2">{environments.map((environment) => <div key={environment.id} className="flex flex-wrap items-center gap-3 rounded-lg border border-border px-4 py-3"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{environment.name}</p><p className="mt-1 text-xs text-muted-foreground">{environment.fileCount} 个文件</p></div><Button variant="ghost" size="sm" onClick={() => setCaptureTarget(environment.id)} disabled={!canEdit}><RefreshCw className="size-4" />捕获更新</Button><Button variant="outline" size="sm" onClick={() => void preparePlan(environment.id)} disabled={!canEdit}><Check className="size-4" />应用</Button><Button variant="ghost" size="sm" onClick={() => { setNewMode("copy"); setCopySource(environment.id); setNewDialogOpen(true); }} disabled={!canEdit}><Copy className="size-4" />复制</Button></div>)}</div>
