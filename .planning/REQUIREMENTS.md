...
- [ ] **ENV-01**: 环境标签页 — 项目环境内页顶部横向标签页切换环境，标签页超出宽度可左右滚动
- [ ] **ENV-02**: 管理环境 — 点击"管理环境"按钮打开模态窗，顶部新增按钮，下方列表四列（名称/创建时间/修改时间/操作），操作包含重命名和删除按钮，删除有二次确认弹窗
- [ ] **ENV-03**: 添加文件 — 点击"添加"按钮向当前标签页环境添加需要管理的配置文件
- [ ] **ENV-04**: 删除文件 — 点击"删除"按钮可列表多选不需要管理的文件，删除有二次确认弹窗
- [ ] **ENV-05**: 同步差异 — 点击"同步差异"按钮（需勾选当前标签页文件才可点击），弹窗显示全部其他环境可多选，选择后出现 IDE Git 冲突风格对比模态窗（使用 `@git-diff-view/react`）
- [ ] **ENV-06**: 文件列表 — 四列（勾选框 / 文件名+后缀 / 修改时间 / 操作），操作列每行有查看按钮
- [ ] **ENV-07**: 文件查看编辑 — 查看按钮打开文本模态窗，支持 xml/yml/json/toml 等多种配置文件语法高亮（CodeMirror 6）+ 语法错误检测，右下角保存按钮
- [ ] **ENV-08**: 环境切换栏 — 项目指令（现项目环境）一行上方新增一行，包含下拉框（选择环境）和启用按钮，点击启用确认切换到所选环境
