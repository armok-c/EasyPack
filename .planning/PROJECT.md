...
## Current Milestone: v2.1 项目环境管理

**Goal:** 去掉全局指令系统，替换为项目环境管理功能，支持项目多环境配置文件的增删改查、切换与差异同步。

### Target Features

1. 打包全局版本号更新 — 构建时自动更新应用版本号
2. 移除全局指令 — 去掉全局指令栏目及相关功能
3. 项目指令替代全局指令 — 原"项目指令"改名"项目环境"，占据全局指令的原位置
4. 项目环境管理功能 — 管理项目的多套配置文件（标签页切换、文件管理、差异同步、语法高亮编辑）
5. 环境切换栏 — 项目指令上方新增下拉框 + 启用按钮

### Active

- [ ] **VER-05**: 打包时自动更新应用版本号
- [ ] **CMD-09**: 移除全局指令栏目及相关功能
- [ ] **ENV-01**: 项目指令栏目移至全局指令位置，改名为"项目环境"
- [ ] **ENV-02**: 项目环境标签页管理，支持左右滚动切换环境
- [ ] **ENV-03**: 管理环境模态窗 — 新增/重命名/删除环境标签（二次确认删除）
- [ ] **ENV-04**: 向当前环境添加需要管理的配置文件
- [ ] **ENV-05**: 删除不需要管理的文件（多选 + 二次确认）
- [ ] **ENV-06**: 同步差异 — 勾选文件后选择目标环境，IDE Git 冲突风格差异对比
- [ ] **ENV-07**: 文件列表（勾选框 / 文件名+后缀 / 修改时间 / 查看按钮）
- [ ] **ENV-08**: 查看文件 — 文本模态窗，支持 xml/yml 等多种语法高亮，右下角保存按钮
- [ ] **ENV-09**: 项目指令上方新增下拉框 + 启用按钮，切换当前项目启用的环境
...
---
*Last updated: 2026-06-16 after starting v2.1 milestone*
