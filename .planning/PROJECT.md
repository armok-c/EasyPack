...
- CMD-09: 修复命令执行错误（0x80070002），所有项目点击指令卡片均无法执行 — v1.1
- UI-08: 无边框窗口 + 自定义标题栏（应用名称拖拽区 + 最小化/最大化/关闭按钮） — v1.1
- PROJ-07: 项目图标增强 — 从项目目录自动识别应用图标 + 自定义选择图标路径 — v1.1
- UI-09: 模态窗自适应大小，窗口过小时模态窗显示不全 — v1.1
- CMD-10: 全局/项目指令切换改为按钮样式，同一行增加"打开文件夹"按钮 — v1.1
- PROJ-08: 指令卡片上方显示文件夹大小 + Git 分支名 — v1.1
- CMD-11: 预设指令系统 — 默认仅保留 git pull 和 claude，双下拉框选择预设（python/pip/git/rust/npm），用户选择全局或项目级 — v1.1
...
## Current Milestone: v1.1 体验增强与预设指令

**Goal:** 修复命令执行核心问题，提升窗口体验和操作效率，引入预设指令系统降低配置成本。
...
- 指令切换按钮化 + 打开文件夹按钮
- 文件夹大小 + Git 分支显示
- 预设指令系统（双下拉框选择，涵盖 python/pip/git/rust/npm）

---

*Last updated: 2026-04-15 starting v1.1 milestone*
