...
## Current Milestone: v2.0 能力跃升

**Goal:** 将 EasyPack 从"单行指令执行器"升级为"多行脚本 + 完整快捷键 + 多配置管理"的强大桌面工具。

**Target features:**
1. **多行脚本指令** — 多行顺序执行 + 条件/流程控制（if/else、变量、错误处理）
2. **应用版本管理** — 版本号显示 + GitHub Releases 更新检查
3. **完整快捷键面板** — VS Code 风格快捷键自定义面板
4. **悬浮窗改进** — 更小更紧凑，可折叠，折叠态显示项目名+图标，点击名称切换项目
5. **开机启动** — 可配置开关，启动后最小化到托盘
6. **多配置文件管理** — 多套独立配置切换 + 导入/导出
...
- **Shipped:** v1.0 MVP (2026-04-15), v1.1 体验增强 (2026-04-26), v1.2 快捷键/托盘/窗口 (2026-05-12)
- **Tech stack:** Tauri 2 + React 19 + TypeScript 5.7 + Tailwind CSS 4 + shadcn/ui + Rust
- **Codebase:** ~9,300 LOC (8,535 TypeScript + 799 Rust), ~240+ files
- **Persistence:** tauri-plugin-store (JSON)
- **Command execution:** Rust std::process::Command with raw_arg → cmd.exe
- **Window:** Frameless + custom TitleBar (startDragging + onMoved debounce)
- **Test infrastructure:** Vitest (168 unit tests), cargo test (17 Rust tests)
...
---
*Last updated: 2026-05-13 starting v2.0 milestone*
